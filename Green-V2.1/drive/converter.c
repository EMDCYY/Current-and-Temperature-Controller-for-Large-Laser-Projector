...
//   return ((DecValue_H << 8) & 0x0F00) + ((DecValue_M << 4) & 0x00F0) + (DecValue_L & 0x000F);
  ...
  DecValue_H = data * 0x960 / 0xFFF / 0x64; //Minimun Current is 0.01A, Maxmum Current is 3.3A. Make data amplify to 330 can get integra after induce 0xFFF 
  DecValue_L = data * 0x960 / 0xFFF % 0x64;
...
int dec2hex(uint8_t data_H, uint8_t data_L)
{
  uint16_t HexValue_H;
  uint16_t HexValue_L;
  HexValue_H = data_H * 0x64 * 0xFFF / 0x960;
  HexValue_L = data_L * 0xFFF / 0x960; 
