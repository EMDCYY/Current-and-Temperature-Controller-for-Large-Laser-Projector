...
//   Dir_Transmit();
//   delay_ms(1);	
//   USART_SendData(USART1, CurrentTuneValue>>8);
//   delay_ms(2);	
//   USART_SendData(USART1, CurrentTuneValue);
//   delay_ms(2);
//   USART_SendData(USART1, ctrlSoftTune>>8);
//   delay_ms(2);	
//   USART_SendData(USART1, ctrlSoftTune);
//   delay_ms(2);
//   Dir_Receive();
...
//   {
    ...
    delay_ms(10);
    Enable_On();
    while(ADC_Data[3]<CurrentTuneValue)
    {
...
void ShutDown(void)
{
  if(ADC_Data[3]<=CurrentMinum && Flag_On==false)
  {
    //
  }
  else if(ADC_Data[3]<=CurrentMinum  && Flag_On==true)
  {
    Enable_Off();
  }
  else if(ADC_Data[3]>CurrentMinum)
  {
    while(ADC_Data[3]>CurrentMinum)
    {
