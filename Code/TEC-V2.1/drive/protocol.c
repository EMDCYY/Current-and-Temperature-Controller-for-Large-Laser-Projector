...
#define COLOR		0x0B   //0x0A - Red; 0x0B - Green; 0x0C - Blue; 0x00 - Check

#define RedColor	0x0A
#define GreenColor  0x0B
...
void wr_TEC(void)
{
    TECTuneValue = dec2hex(rd[9], rd[10]); 	
    Response(Error);
  	Flag_Order = true;
}
...
// 	wd[12]= (ADC_Data[5] & 0x00FF);
  	ConverterData = hex2dec(ADC_Data[3]);
...
// 	wd[14]= (CurrentTuneValue & 0x00FF);
    ...
    wd[15]= 0x00;
    wd[16]= 0x00;
// 	wd[15]= (ADC_Data[2] & 0xFF00)>>8;
// 	wd[16]= (ADC_Data[2] & 0x00FF);
...
bool StartUp_Condition(void)
{
	if(Flag_Order == true)
	{
		if(ADC_Data[3] <=0x0010 && rd[SN_Order] ==0x01)
		{
			return true;
		}
		else
...
bool ShutDown_Condition(void)
{
	if(Flag_Order == true)
	{
		if(ADC_Data[3] >0x0010 && rd[SN_Order] ==0x02)
		{
			return true;
		}
		else
...
bool Tune_Condition(void)
{
	if(Flag_Order == true)
	{
		if(ADC_Data[3] >0x0010 && rd[SN_Order] ==Order_wr_Current && rd[SN_Data_1]==PCX && rd[SN_Data_2]==PCX)
		{
			return true;
		}
		else
